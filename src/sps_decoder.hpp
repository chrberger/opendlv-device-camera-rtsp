// Found here: https://stackoverflow.com/questions/12018535/get-the-width-height-of-the-video-from-h-264-nalu
//
// License CC-BY-SA 3.0
//
// Author: Pankaj Vavadiya, https://stackoverflow.com/users/3963057/pankaj-vavadiya
...
struct FrameDimension Parse(const unsigned char * pStart, unsigned short nLen)
{
    m_pStart = pStart;
    m_nLength = nLen;
    m_nCurrentBit = 0;
    ...
    int Width = ((pic_width_in_mbs_minus1 +1)*16) - frame_crop_bottom_offset*2 - frame_crop_left_offset*2;
    int Height = ((2 - frame_mbs_only_flag)* (pic_height_in_map_units_minus1 +1) * 16) - (frame_crop_right_offset * 2) - (frame_crop_top_offset * 2);

    //printf("\n\nWxH = %dx%d\n\n",Width,Height);

    struct FrameDimension fd;
    fd.width = Width;
    fd.height = Height;
    return fd;
}
